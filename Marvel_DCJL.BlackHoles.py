...
if len(Data[sim])>0:
    ...
    with pymp.Parallel(args.numproc) as pl:
        for i in pl.xrange(len(Sims[sim]['halos'])):
            ...
            black_holes = halo.s[(halo.s['tform']<0)]
            distances = []
            for index in np.arange(len(black_holes)):
                current[f'BH{index+1}'] = {}
                current[f'BH{index+1}']['mass'] = black_holes['mass'][index]
                current[f'BH{index+1}']['pos'] = black_holes['pos'][index]
                current[f'BH{index+1}']['vel'] = black_holes['vel'][index]
                current[f'BH{index+1}']['central'] = True if len(black_holes)==1 else False
                distances.append(np.linalg.norm(black_holes['pos'][index]))
            if len(black_holes)>1:
                current[f'BH{distances.index(min(distances))+1}']['central'] = True
            ########################################################
